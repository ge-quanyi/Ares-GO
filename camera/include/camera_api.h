...
struct camera_config {
    ...
    char *SN;
    //设置ROI的宽度
    int nWidth = 960;//960                  960          640             480
    //设置ROI的高度
    int nHeight = 768;//768                 768           640
    //设置ROI的X方向偏移
    int nOffsetX = 160;//160                160           320
    //设置ROI的Y方向偏移
    int nOffsetY = 128;//128                128           192          272
    //是否开启触发模式
    int TriggerMode = 0;
    //设置触发源
    int TriggerSource = 1;
    ...
    int GainAuto = 1;
    //选择增益通道
    int GainSelector = 0;          //0:all 1:red 2:green 3:blue
    //设置增益值
    double Gain = 15.0;
};
