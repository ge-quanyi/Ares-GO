...
Inference::Inference(const std::string& path) : model_path_(path)
{
    initModel();
}
...
bool Inference::initModel()
{
    ...
    network = ie.ReadNetwork(model_path_);
...
bool Inference::detect(Mat &src,std::vector<ArmorObject>& objects)
{
    if (src.empty())
    {
